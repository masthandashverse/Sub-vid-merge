...
if __name__ == '__main__':
    ...
    print("=" * 55)
    ok = check_ffmpeg()
    print(f"  FFmpeg  : {'✅ found' if ok else '❌ NOT FOUND — install FFmpeg!'}")
    if ok:
        for k, v in check_filters().items():
            print(f"  {k:12s}: {'✅' if v else '❌'}")
    print(f"  Output  : {os.path.abspath(app.config['OUTPUT_FOLDER'])}")
    print(f"\n  Open    : http://127.0.0.1:5000")
    print(f"  Debug   : http://127.0.0.1:5000/debug")
    print("=" * 55)
